...
DEFAULT_STAGE_ROOT = SEARCH_ROOT / "staging" / "gauss-index-batch-20260328-400"
DEFAULT_OWNER = "gauss"
...
def make_candidates() -> List[CandidateSpec]:
    ...
    idx = 0
    for family, dot_variants in [("base", BASE_DOT_VARIANTS), ("extended", EXTRA_DOT_VARIANTS)]:
        for q_variant, k_variant, dot_variant, accum_variant in product(
            Q_VARIANTS, K_VARIANTS, dot_variants, ACCUM_VARIANTS
        ):
            idx += 1
            out.append(
                CandidateSpec(
...
def _head_major(q: torch.Tensor) -> torch.Tensor:
...
def _dot_kernel(q: torch.Tensor, k: torch.Tensor, variant: str) -> torch.Tensor:
    ...
    if variant == "einsum_kq":
        return torch.einsum("bnd,bmhd->bmnh", k32, q32)
    if variant == "einsum_qk":
        return torch.einsum("bmhd,bnd->bmnh", q32, k32)
    if variant == "matmul_broadcast":
        return torch.matmul(k32.unsqueeze(1), q32.permute(0, 1, 3, 2))
    if variant == "bmm_broadcast":
        b, m, h, d = q32.shape
        n = k32.size(1)
        lhs = k32.unsqueeze(1).expand(-1, m, -1, -1).reshape(b * m, n, d)
        rhs = q32.reshape(b * m, h, d).transpose(1, 2)
        return torch.bmm(lhs, rhs).view(b, m, n, h)
    if variant == "broadcast_reduce":
        logits = q32.unsqueeze(3) * k32.unsqueeze(1).unsqueeze(1)
        return logits.sum(dim=-1, dtype=torch.float32).permute(0, 1, 3, 2)
    if variant == "einsum_head_major":
        return torch.einsum("bhmd,bnd->bhmn", _head_major(q32), k32).permute(0, 2, 3, 1)
    if variant == "matmul_head_major":
        return torch.matmul(_head_major(q32), k32.transpose(1, 2)).permute(0, 2, 3, 1)
    if variant == "bmm_head_major":
        qh = _head_major(q32)
        b, h, m, d = qh.shape
        n = k32.size(1)
        lhs = qh.reshape(b * h, m, d)
        rhs = k32.unsqueeze(1).expand(-1, h, -1, -1).reshape(b * h, n, d).transpose(1, 2)
        return torch.bmm(lhs, rhs).view(b, h, m, n).permute(0, 2, 3, 1)
    if variant == "broadcast_reduce_head_major":
        qh = _head_major(q32)
        logits = qh.unsqueeze(3) * k32.unsqueeze(1).unsqueeze(1)
        return logits.sum(dim=-1, dtype=torch.float32).permute(0, 2, 3, 1)
    if variant == "matmul_flat_heads":
        qh = _head_major(q32)
        b, h, m, d = qh.shape
        n = k32.size(1)
        lhs = k32.unsqueeze(1).expand(-1, h, -1, -1).reshape(b * h, n, d)
        rhs = qh.reshape(b * h, m, d).transpose(1, 2)
        return torch.matmul(lhs, rhs).view(b, h, n, m).permute(0, 3, 2, 1)
...
def build_queue_manifest(
    ...
    manifest = {
        ...
        "command": (
            ...
            "python3 search/index_400_sweep.py run "
            f"--shape {shape_name} "
            f"{candidate_flags} "
...
def _is_statically_valid(shape_name: str, candidate: CandidateSpec) -> bool:
    if shape_name == "decode":
        return True
    if candidate.dot_variant in {"broadcast_reduce", "broadcast_reduce_head_major"}:
        return False
    return True
    ...
    readme.write_text(
        "\n".join(
            [
                ...
                "- 400 candidate variants per shape",
                "- 200 base exact formulation candidates and 200 extended exact formulation candidates",
                ...
                "",
                "To inspect a manifest, open `manifests/<shape>/<shard>.json`.",
            ]
        )
        + "\n"
    )

    family_counts = {"base": 0, "extended": 0}
    for candidate in candidates:
...
def main() -> None:
    ...
    run.add_argument("--candidate-limit", type=int, default=400)
    ...
    run.add_argument("--list-candidates", action="store_true")

    for name in ["stage", "queue"]:
    ...
    if args.cmd == "run":
        if args.list_candidates:
            print(json.dumps([asdict(candidate) | {"label": candidate.label} for candidate in make_candidates()], indent=2))
            return
        if torch is None:
            raise SystemExit("torch is required for run mode")
